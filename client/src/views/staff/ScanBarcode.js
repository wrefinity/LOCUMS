...
function Sale({ cart, setCart }) {
  ...
  function addScanned(x) {
    ...
    let cartItem = {
      ...
      type: temp.variations[temp.key].type,
      barcode: scannerInput,
      price:
        selling_price > 0
          ? selling_price
          : temp.variations[temp.key].selling_price,
    };
    let found = 0;

    if (temp.variations[temp.key].type === "Type 1") {
      found = cart.findIndex(
        (x) =>
          x.variation === cartItem.variation &&
    ...
    if (found > -1) {
      ...
      setNotificationDetails({
        msg: "Item already already exist in cart, updated with new quantity",
        type: "info",
      });
      setNotificationStatus(true);
    } else {
      setCart([...cart, cartItem]);
      setCurrentProduct({ ...x, toBuy: 1 });
  ...
  async function scanBarcode(e) {
    ...
    await axios
      .get(product.scanBarcode + "/" + id + "/" + scannerInput)
      .then((response) => {
        if (response.data.status === true) {
          ...
          setNotificationDetails({
            msg: "Error finding scanned item!",
            type: "danger",
          });
          setNotificationStatus(true);
        }
                          ...
                          >
                            ...
                            <Col className="pr-md-1" md="12">
                              <FormGroup>
                                ...
                                {/* Model:{" "}
                                {
                                  currentProduct.variations[currentProduct.key]
                                    .model
                                }{" "} */}
                                <br />
                                Selling Price:{" "}
